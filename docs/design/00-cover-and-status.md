...
| One-month target | Demonstrate an integrated edge-first system | Camera and barcode adapters, local database/media, edge API/dashboard, product windows, per-component aggregation, persistent upload queue, initial central API/database/dashboard, Compose deployment, resilience and acceptance evaluation | Unvalidated production guarantees; automated retraining; Kubernetes dependency | Proposed; sequencing depends on hardware and site access |
