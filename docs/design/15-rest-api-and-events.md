...
| `GET /api/v1/device/status` | Full device, disk, network, and queue status. Includes host system metrics for the health view: CPU count, 1-minute load average, memory total/available bytes, storage volume total/free bytes, network receive/transmit rates (bytes/second), and NVIDIA GPU utilization and power. Metric values unavailable on a platform are `null`, never fabricated. | None / `DeviceStatus` | `503 STATUS_UNAVAILABLE` | Safe GET | - | R |
