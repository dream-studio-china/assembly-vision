...
1. Verify and record camera mount, position, focus, exposure, lighting,
   ...
   UPS where applicable, and device identifiers. For a GigE Vision / GenICam
   camera, record serial number, model, firmware, GenTL producer, network
   interface, pixel format, trigger mode, packet size, and whether jumbo frames
   were validated. For a USB camera, record UVC/driver identity and device path.
