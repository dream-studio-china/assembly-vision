...
def _drift(
    ...
    **params: Any,
    ...
    query: dict[str, Any] = {"product_code": product_code, "rule_version_id": rule_version_id}
    query.update(params)
...
def test_confidence_drift_isolation_across_product_and_rule(tmp_path: Path) -> None:
    ...
    with _make_client(root, tmp_path) as client:
        ...
        assert today["evidence_count"] == 2
        assert today["weighted_mean"] == pytest.approx(13.0 / 15.0)
        # The unfiltered report includes everything on the device.
        unfiltered = client.get("/api/v1/statistics/confidence-drift").json()
        assert unfiltered["periods"]["today"]["evidence_count"] == 4
        assert unfiltered["periods"]["today"]["weighted_mean"] == pytest.approx(22.0 / 35.0)
