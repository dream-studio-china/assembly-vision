"""Container health check for the edge service (design 20.4, E5a).

Usage::

    python -m assemblyvision.healthcheck <url>
...
def check(url: str) -> bool:
    ...
    try:
        parts = urlsplit(url)
        # Only http(s) targets are meaningful for a health check; reject
        # file: or custom schemes before any request is made.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print("usage: python -m assemblyvision.healthcheck <url>", file=sys.stderr)
        return 2
