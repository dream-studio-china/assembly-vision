...
def load_pipeline_config(path: Path) -> PipelineConfig:
    ...
    product_detection = DetectionSettings(
        ...
        confidence_threshold=_as_number(product_detection_raw.get("confidence_threshold", 0.7), "product_detection.confidence_threshold"),
        iou_threshold=_as_number(product_detection_raw.get("iou_threshold", 0.5), "product_detection.iou_threshold"),
    )
    component_detection = DetectionSettings(
        model_version=_require_str(component_detection_raw.get("model_version"), "component_detection.model_version"),
        confidence_threshold=0.0,
        iou_threshold=_as_number(component_detection_raw.get("iou_threshold", 0.5), "component_detection.iou_threshold"),
    )
    ...
    for code, settings_raw in components_raw.items():
        ...
        components[code] = ComponentDetectionSettings(
            observation_threshold=_as_number(
                settings.get("observation_threshold", 0.5), f"component_detection.components.{code}.observation_threshold"
            )
        )
    if not components:
    ...
    try:
        roi = ROIConfig(
            ...
            min_area_pixels=int(_as_number(roi_raw.get("min_area_pixels", 250000), "roi.min_area_pixels")),
            min_expanded_area_retained=_as_number(roi_raw.get("min_expanded_area_retained", 0.90), "roi.min_expanded_area_retained"),
            normalize_perspective=bool(roi_raw.get("normalize_perspective", False)),
        )
    except ConfigError:
        raise
