...
<template>
  <section class="review-panel">
    <h3>{{ t("Human review") }}</h3>
    <el-alert
      v-if="props.businessResult === 'NG'"
      :title="t('This inspection can be reviewed')"
      :description="t('Review the machine decision against the retained evidence and record a disposition. The machine decision is never rewritten.')"
      type="warning"
      show-icon
      :closable="false"
    />
    <el-alert
      v-else
      :title="t('Optional audit review')"
      :description="t('Confirm the OK result or correct it to NG when evidence shows a missed defect.')"
      type="info"
      show-icon
      :closable="false"
    />
    ...
    <el-alert
      v-if="historyError"
      :title="t('Review history could not be loaded: {error}', { error: historyError })"
      type="error"
      show-icon
      :closable="false"
    >
      <template #default>
        <p class="review-panel__history-error">
          {{ t("The existing dispositions are unknown, so a new review would silently supersede them. Reload the history before submitting.") }}
        </p>
        <el-button size="small" :loading="loading" @click="load">{{ t("Reload review history") }}</el-button>
      </template>
    </el-alert>
    ...
    <div class="review-panel__form">
      ...
      >
        <el-option
          ...
          :key="d"
          :value="d"
          :label="t(DISPOSITION_LABELS[d])"
        />
      </el-select>
      <el-input
        v-model="reviewer"
        :placeholder="t('Reviewer name (required)')"
        :aria-label="t('Reviewer name (required)')"
        style="width: 220px"
      />
      <el-input
        v-model="reason"
        :placeholder="needsReason ? t('Reason (required for inconclusive)') : t('Reason (optional)')"
    ...
    <h4 v-if="reviews.length">{{ t("Review history (append-only)") }}</h4>
    ...
    <p v-else-if="!loading && !historyError" class="review-panel__empty">{{ t("No reviews recorded.") }}</p>
  </section>
</template>
