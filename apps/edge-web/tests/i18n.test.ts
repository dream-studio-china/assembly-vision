...
describe("i18n locale catalogs", () => {
  ...
  it("translates spot-checked strings per locale", () => {
    const samples: Array<[string, Record<string, string>]> = [
      ["en", { History: "History", "No inspections found": "No inspections found" }],
      ["zh-CN", { History: "历史记录", "No inspections found": "未找到检验记录" }],
      ["zh-HK", { History: "歷史記錄", "No inspections found": "找不到檢驗記錄" }],
      ["ja", { History: "履歴", "No inspections found": "検査記録が見つかりません" }],
    ];
  ...
  it("resolves interpolation parameters in every locale", () => {
    for (const locale of ["en", "zh-CN", "zh-HK", "ja"]) {
      applyLocale(locale as (typeof SUPPORTED_LOCALES)[number]["value"]);
      const value = i18n.global.t("Disk: {bytes} free", { bytes: "1.2 GB" });
      expect(value).toContain("1.2 GB");
    }
  });

  it("renders literal @ inside messages (escaped, not linked)", () => {
    // vue-i18n treats `@` as the linked-message prefix, so the fps detail
    // message must escape it and still render the literal character.
    for (const locale of ["en", "zh-CN", "zh-HK", "ja"]) {
      applyLocale(locale as (typeof SUPPORTED_LOCALES)[number]["value"]);
      const value = i18n.global.t("{width}×{height} {'@'} {fps} fps", {
        width: 1280,
        height: 1024,
        fps: 30,
      });
      expect(value).toContain("1280×1024");
...
describe("locale selection", () => {
  ...
  it("persists the selection on applyLocale", () => {
    ...
    expect(written["assemblyvision.edge.locale"]).toBe("zh-CN");
