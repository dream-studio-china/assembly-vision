...
def test_component_hints_suggest_rule_bump(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    _print_improvement_hints("COMPONENT_DETECTION", Path("models/weights/component-yolo-0.2.0.pt"), rule)
...
def test_product_hints_mention_roi_regeneration(capsys: pytest.CaptureFixture[str]) -> None:
    _print_improvement_hints("PRODUCT_DETECTION", Path("models/weights/product-yolo-0.2.0.pt"), None)
    out = capsys.readouterr().out
